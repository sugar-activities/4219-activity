...
try:  
    new_screen_size = (display_info.current_w,display_info.current_h-70)
...
def stop_facility(facility_obj):
    ...
    message.push_message(_('Facility ')+model.FACILITY_NAMES[facility_obj.get_name()]+_(' has been temporarily stopped due to insufficient resources to run the facility'),'high')
    ...
    message.push_message(_('Facility ')+model.FACILITY_NAMES[facility_obj.get_name()]+_(' has been resumed'),'low')
...
def get_setup_text(facility_obj):
    ...
    text +=_('Resources required to build :  BUILDING MATERIAL:')+str(int(cost_build['BUILDING MATERIAL']))+_(' TOOLS :')+str(int(cost_build['TOOLS']))+_(' WATER :')+str(int(cost_build['WATER']))+'\n'
    cost_run = facility_obj.get_cons_dict()
    if cost_run:
        text +=_('Resources required to run : ')
        for key in cost_run.keys():
            text +=key+': '+str(int(cost_run[key]))+' '
        text +='\n'
    text +=_('Manpower required : To build: ')+str(int(model.FACILITY_MANP_DICT_BUILD[facility_obj.get_name()]['EMPLOYED PEOPLE IN CONSTRUCTION']))+_(' To run: ')
...
def get_upgrade_text(facility_obj):
    ...
    if facility_obj.get_level() < 3:
        ...
        text += _('Resources required to upgrade : BUILDING MATERIAL: ')+str(int(cost_upgrade['BUILDING MATERIAL']))+_(' TOOLS: ')+str(int(cost_upgrade['TOOLS']))
    else:
        text = _('You cannot upgrade the facility anymore, it has reached its maximum level')
...
def build_facility(facility_obj, list_food = model.DEF_FARM_PROD):
    ...
    if facility_obj.get_number() == 0:
        ...
        if facility_obj.get_original_number() > 0:
            text = _('You cannot build a facility when it has been temporarily stopped, try building it when it is resumed')
    ...
    try:
        ...
        text = _('You dont have enough resources to build the facility,  please try later')
        message.push_message(text,'high')
        return text
    except Exceptions.Low_Manpower_Resources_Exception:
        text = _('You dont have enough manpower to build the facility, please try later')
        message.push_message(text,'high')
        return text
    except Exceptions.Maximum_Number_Reached:
        text = _('You cannot setup more buildings of this facility, try setting up some other facility')
        message.push_message(text,'high')
        return text
    ...
    return _('Facility has been build')
...
def upgrade_facility(facility_obj):
    ...
    if facility_obj.get_number() == 0:
        ...
        if facility_obj.get_original_number() > 0:
            text = _('You cannot upgrade a facility when it has been temporarily stopped, try upgrading it when it is resumed')
            message.push_message(text,'high')
            return text
        text = _('You need to setup a facility first to upgrade it')
        message.push_message(text,'high')
        return text
    ...
    try:
        ...
        text =  _("You don't have enough resources to upgrade the facility please try later")
        message.push_message(text,'high')
        return text
    except Exceptions.Maximum_Level_Reached:
        text =  _('Facility has reached its maximum level you cant upgrade it now')
        message.push_message(text,'high')
        return text
    ...
    text = _('Facility has been upgraded')
    return text
...
def buy_res(res,res_quantity):
    ...
    try:
        ...
        text =_('You dont have enough money to buy this resource. Please change the quantity or try later')
        return text
    except Exceptions.Resources_Underflow_Exception:
        text =_('The market doesnot have enough quantity to sell this resource to village')
        return text
    except Exceptions.Resources_Overflow_Exception:
        text =_('The Village cannot store so much amount of resources you should try and use the money to buy some other resources ')
        return text
        
    text = _('The Village has bought the resource you demanded')
...
def sell_res(res,res_quantity):
    ...
    try:
        ...
        text = _('The village doesnot have enough quantity to sell this resource to market')
        return text
    except Exceptions.Resources_Overflow_Exception:
        text = _('The Village has sold the resource you demanded')
        return text
    text = _('The Village has sold the resource you demanded')
