...
class chat:
    ...
    def __init__(self):
        ...
        self.label_text='     '+_('ENTER : To show whole chat')+'              '+_('ESC : To skip chat')+'           '
    ...
    def addChat(self,playerName = None,message = ''):
        ...
        #### TODO : This self.myfont is supposed to be replaced by pangofont when the storyboard will be translated, however, right now done like this
              #### as the characters doesn't appear to be coming completer otherwise, so need to solve the problem by the time we are going to localize
              #### the storyboard. 
        self.myfont = pygame.font.Font("font.ttf",25)
        #self.myfont = pangofont.PangoFont(size=25)
        textColor = (0,0,0)
    ...
    if e.type==KEYDOWN:
        ...
        if e.key==K_RETURN:
            if show_whole_chat_flag==False:
                ...
                chatObject.label.text='     '+_('ENTER : To move to next set of chats')+'              '+_('ESC : To skip chat')+'           '
            else:
