...
def messageResponderUnicast(handle,nick,message):
    ...
    if Flag_Trading_Unicast_Acknowledge:
        ...
        if message[0] == 'acknowledge_trade':
            ...
            Flag_Trading_Unicast_Acknowledge = False
            threades.message.push_message(_('The Resources have been traded'),'low')
            
        if message[0] == 'decline_trade':
            threades.message.push_message(_('The Resources have already been traded with someone'),'medium')
    ...
    if Flag_Trading_Unicast_Recieve_Buy:
        if message[0] == 'TradeReply':
            ...
            threades.buy_res(res,quantity,price)
            Flag_Trading_Unicast_Recieve_Buy = False
            threades.message.push_message(_('The Resources have been traded with ')+nick,'low')
...
class meshTrading:
    ...
    def tradingWindow(self,handle,buddyName = 'Friend',resource = 'Water', quantity = '0' ,price = '10',trade = 'sell'):
        ...
        self.win = gui.Window(position = position_win, size = size_win, parent = threades.desktop, text = _("Trade ") ,style = win_style,shadeable = False, moveable = False)
        # Creating label
        ...
        self.button_accept = gui.Button(position = threades.resize_pos((100.0,130.0),(470.0,180.0),size_win), size = threades.resize_pos((100.0,40.0),(470.0,180.0),size_win), parent = self.win, text = _(" Accept "),style = button_style)
        self.button_reject = gui.Button(position = threades.resize_pos((300.0,130.0),(470.0,180.0),size_win), size = threades.resize_pos((100.0,40.0),(470.0,180.0),size_win), parent = self.win, text = _(" Reject "),style = button_style)
